...
if (isset($_POST["reset-request-submit"])) {
  ...
  $url = "http://www.novaprospekt.xyz/create-new-password.php?selector=" . $selector . "&validator=" . bin2hex($token);
  ...
  if (!mysqli_stmt_prepare($stmt, $sql)) {
    ...
    if ($row = mysqli_fetch_assoc($result)) {
      ...
      if ($sa1Check == false) {
        ...
        $mail->Username   = 'AKIA5J45A2YBHTIBDSFR';     // SMTP username
        $mail->Password   = 'BE9n2GcD8mqUgVO5QxyDSWyhy227ByjGccjgNr5FxE4t';                     // SMTP password
        $mail->SMTPSecure = 'tls';                                  // Enable TLS encryption, `ssl` also accepted
        $mail->Port       = 587;                                    // TCP port to connect to

        //Recipients
        $mail->setFrom('novaprospekt.blackmesa@gmail.com', 'Black Mesa Support');
